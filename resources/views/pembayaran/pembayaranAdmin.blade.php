    ...
    <div class="wrapper">
        ...
        <div class="content-wrapper">
            ...
            <section class="content">
                ...
                <div class="container-fluid">
                    <form action="{{ url('/getPembayaranAdmin') }}" method="GET" class="mb-3">
                                    ...
                                    <table class="table table-head-fixed text-nowrap">
                                        ...
                                        <tbody>
                                            @foreach ($pembayaran as $o)
                                                <tr>
                                                    <td>{{ $o->pendaftaran ? $o->pendaftaran->nama_lengkap : '-' }}</td>
                                                    <td>{{ $o->pendaftaran ? $o->pendaftaran->no_pendaftaran : '-' }}</td>
                                                    <td>{{ $o->total_bayar }}</td>

                                                    <td>
                                                        @if ($o->bukti_pembayaran)
                                                            <a href="{{ asset('/storage/img/bukti/' . $o->bukti_pembayaran) }}"
                                                    ...
                                                    <td>{{ $o->digunakan_untuk }}</td>
                                                    <td>{{ $o->tanggal_bayar }}</td>
                                                    ...
                                                    <td>
                                                        <a href="{{ url('/orangtua/view/' . $o->id) }}"
                                                            class="btn btn-warning mb-2"><i class="fa fa-eye"></i>
                                                            Lihat Lebih Lengkap</a>
                                                    </td>
                                                    <td>
                                                        <form action="{{ route('pembayaran.destroy', $o->id) }}"
                                                            method="POST"
        ...
        document.querySelectorAll('.status-select').forEach(select => {
            select.addEventListener('change', function() {
                ...
                if (status === 'ditolak') {
                    ...
                    fetch('{{ route('pembayaran.updateStatus') }}', {
                            method: 'POST',
                            headers: {
